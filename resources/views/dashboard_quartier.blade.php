...
<script type="text/javascript">
 function validate() {
    ...
    var valid = true;
    valid = checkEmpty($("#libelle_quartier"));
    return valid; 
  }
...
<div class="tab-content">
	<div class="row" style="margin-bottom:15px; margin-top:15px;">
          ...
          <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">LISTE DES QUARTIERS</h4>
          </caption>  
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              <th class="text-center">Nom du quartier</th>
            </tr>
          </thead>
           @foreach($quartiers as $item)
              <tr id="item{{$item->id}}">
                ...
                <td class="text-left"><a class="edit-modalA" style="cursor:pointer"
                    data-info="{{$item->id}},{{$item->libelle_quartier}}">
                    <span class="glyphicon glyphicon-edit"></span> {{$item->libelle_quartier}}
                  </a>
                </td>
              </tr>
          @endforeach
      ...
      <div id="add_antibiotique" class="modal fade" role="dialog">
				  <div class="modal-dialog">
                    ...
                    <div class="form-group ">
                      <label for="libelle_antibiotique" class="control-label col-sm-5">Nom du Quartier (*):&nbsp;<br /><span class="libelle_quartier-validation validation-error"></span></label>
                      <div class="col-sm-6">
                        <input  class="form-control" name="libelle_quartier" id="libelle_quartier" type="text" />
                      </div>
                    </div>
                    
                  </div>
                  
                  <input type="hidden" id="id_quartier" name="id_quartier" value="">
				        </form>
				      </div>
   ...
   $('div.dataTables_filter input').focus();
     ...
     $('form input').on('keypress', function(e) {
        ...
        $(document).on('click', '.edit-modalA', function() {
            ...
            $('.modal-titleA').text("Modifier les informations du quartier");
            
            
            var details = $(this).data('info').split(',');
            $('#form-antibiotique #id_quartier').val(details[0]);
    
            $('#form-antibiotique #libelle_quartier').val(details[1]);    
         ...
         $(document).on('click', '#addA', function() {
            ...
            $('.modal-titleA').text('Ajouter un nouveau Quartier');
    ...
    $(this).attr("disabled","disabled");
      ...
      .done(function(msg){
                        if(typeof msg['erreur'] !== 'undefined'){
                             $(":input:not('[name=_token],[type=checkbox],[type=radio]')").val('');
                              ...
                              var data = [
                               "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_quartier + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_quartier + "</a>"
                              ];
     ...
     $('#footerA').on('click', '.edit', function() {
      ...
      .done(function(msg){
                        ...
                        tr.find('td:eq(0)').html(  "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_quartier + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_quartier + "</a>");
