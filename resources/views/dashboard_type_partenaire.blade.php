...
<div class="tab-content">
  <div class="row" style="margin-bottom:15px; margin-top:15px;">
          ...
          <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">LISTE DES TYPES PARTENAIRES</h4>
          </caption>  
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              <th class="text-center">Nom du Type Partenaire</th>
            </tr>
          </thead>
           @foreach($type_partenaires as $item)
              <tr id="item{{$item->id}}">
                
                <td class="text-left"><a class="edit-modalA" style="cursor:pointer"
                    data-info="{{$item->id}},{{$item->libelle_type_partenaire}}">
                    <span class="glyphicon glyphicon-edit"></span> {{$item->libelle_type_partenaire}}
                  </a>
                </td>
              </tr>
          @endforeach
          ...
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              <div class="modal-header" style="background-color:rgb(103,184,120); color:white;">
                ...
                <h2 id="titre" class="modal-titleA" >Ajouter un Type partenaire</h2>
              </div>
              <div class="modal-body">
                <form  id="form-antifongique" class="form-horizontal" >
                  ...
                  <div class="row">
                      ...
                      <label for="libelle_type_partenaire" class="control-label col-sm-4">Nom du Type Partenaire :</label>
                      <div class="col-sm-7">
                        <input  class="form-control " name="nom_type_partenaire" id="libelle_type_partenaire" type="text" />  
                      </div>
                      
                      </div>
                  </div>
                  
                  <input type="hidden" id="id_type_partenaire" name="id_type_partenaire" value="">
                </form>
              </div>
...
@section('another')
    ...
    $(".dataTables_filter input").addClass("form-control");
     ...
     $('form input').on('keypress', function(e) {
        ...
        $(document).on('click', '.edit-modalA', function() {
            ...
            $('.modal-titleA').text('Modifier les informations du Type Partenaire');
            
            
            var details = $(this).data('info').split(',');
            $('#form-antifongique #id_type_partenaire').val(details[0]);
    
            $('#form-antifongique #libelle_type_partenaire').val(details[1]);    
         ...
         $(document).on('click', '#addA', function() {
            ...
            $('.modal-titleA').text('Ajouter un nouveau Type Partenaire');
  ...
  $('#footerA').on('click', '.ajout', function() {
        $.ajax({
            type: 'post',
            dataType:'json',
            url: "{{route('dashboard_type_partenaire')}}",
      ...
      .done(function(msg){
                        if(typeof msg['erreur'] !== 'undefined'){
                             $(":input:not('[name=_token],[type=checkbox],[type=radio]')").val('');
                              ...
                              var data = [
                               "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_type_partenaire + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_type_partenaire + "</a>"
                              ];
     ...
     $('#footerA').on('click', '.edit', function() {
      ...
      .done(function(msg){
                        ...
                        tr.find('td:eq(0)').html(  "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_type_partenaire + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_type_partenaire + "</a>");
