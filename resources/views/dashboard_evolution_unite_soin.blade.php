...
<div class="tab-content">
  ...
  <table id="tabEPM" style="width:100%; heigth:60%" class="table table-striped table-responsive blockMe">
           <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">EVOLUTION UNITE DE SOINS</h4>
          </caption>   
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              ...
              <th class="text-left">Unité de soins</th>
...
@endsection
@section('another')
  var  tableD =  $('#tabEPM').DataTable({
                "aLengthMenu": [[5, 10, 15, -1], [5, 10, 15, "All"]],
                "iDisplayLength": 10,
                 "order": [[ 0, "asc" ]],
               "serverSide": true,
               "ajax" : {
                  "url" : "{{ route('dashboard_get_evolution_unite_soins') }}",
                  "dataType" : "json",
                  "type" : "POST",
                   "data": function ( d ) {
                    return $.extend( {}, d, {
    ...
    $("#btnOk").click(function(){
     ...
     $('#btnPrintA').click(function(){
          if( ($("#tabEPM").dataTable().fnSettings().aoData.length!==0) ){
            window.open("{{ route('dashboard_evolution_unite_soins_impression') }}?annee_debut="+ $("#annee_debut").val() +"&annee_fin="+$("#annee_fin").val());
          }else{
              $("#AlerPre1").modal({
                        keyboard: false,
                        show : true,
