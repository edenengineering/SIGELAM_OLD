...
<div class="tab-content">
  <div class="row" style="margin-bottom:15px; margin-top:15px;">
          ...
          <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">LISTE DES TYPES CONCLUSIONS</h4>
          </caption>  
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              <th class="text-center">Nom du Type Conclusion</th>
            </tr>
          </thead>
           @foreach($type_conclusions as $item)
              <tr id="item{{$item->id}}">
          ...
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              <div class="modal-header" style="background-color:rgb(103,184,120); color:white;">
                ...
                <h2 id="titre" class="modal-titleT" >Ajouter un Type Conclusion</h2>
              </div>
                ...
                <form  id="form-TC" class="form-horizontal" >
                  ...
                  <div class="row">
                    <div class="form-group ">
                      <label for="libelle" class="control-label col-sm-3">Nom du Type Conclusion (*):<br /><span class="nom_type_conclusion-validation validation-error"></span></label>
                      <div class="col-sm-7">
                        <input  class="form-control" name="nom_type_conclusion" id="libelle" type="text" />   
                      </div>
                    
                    </div>
                  </div>
                  
                  <input type="hidden" id="id_type_conclusion" name="id_type_conclusion" value="">
                </form>
              </div>
    ...
    $(".dataTables_filter input").addClass("form-control");
     ...
     $('form input').on('keypress', function(e) {
        ...
        $(document).on('click', '.edit-modalTC', function() {
            ...
            $('.modal-titleT').text('Modifier les informations du Type Conclusion');
            
            
            var details = $(this).data('info').split(',');
            $('#form-TC #id_type_conclusion').val(details[0]);
         ...
         $(document).on('click', '#addTC', function() {
            ...
            $('.modal-titleT').text('Ajouter un nouveau Type Conclusion');
    ...
    if(validate()){
       $(this).attr("disabled","disabled");
        $.ajax({
            type: 'post',
            dataType:'json',
            url: "{{route('dashboard_type_conclusion')}}",
