...
<script type="text/javascript">
 function validate() {
    ...
    var valid = true;
    valid = checkEmpty($("#libelle_nature"));
    return valid; 
  }
...
<div class="tab-content">
	<div class="row" style="margin-bottom:15px; margin-top:15px;">
          ...
          <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">LISTE DES NATURES</h4>
          </caption>  
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              <th class="text-center">Nature de l'echantillon</th>
            </tr>
          </thead>
           @foreach($natures as $item)
              <tr id="item{{$item->id}}">
                ...
                <td class="text-left"><a class="edit-modalA" style="cursor:pointer"
                    data-info="{{$item->id}},{{$item->libelle_nature}}">
                    <span class="glyphicon glyphicon-edit"></span> {{$item->libelle_nature}}
                  </a>
                </td>
              </tr>
          @endforeach
      ...
      <div id="add_antibiotique" class="modal fade" role="dialog">
				  <div class="modal-dialog">
                    ...
                    <div class="form-group ">
                      <label for="libelle_antibiotique" class="control-label col-sm-4">Nature de l'echantillon (*):&nbsp;<br /><span class="libelle_nature-validation validation-error"></span></label>
                      <div class="col-sm-7">
                        <input  class="form-control" name="libelle_nature" id="libelle_nature" type="text" />
                      </div>
                    </div>
                    
                  </div>
                  
                  <input type="hidden" id="id_nature" name="id_nature" value="">
				        </form>
				      </div>
    ...
    $(".dataTables_filter input").addClass("form-control");
     ...
     $('form input').on('keypress', function(e) {
        ...
        $(document).on('click', '.edit-modalA', function() {
            ...
            $('.modal-titleA').text("Modifier les informations de la nature de l'echantillon");
            
            
            var details = $(this).data('info').split(',');
            $('#form-antibiotique #id_nature').val(details[0]);
    
            $('#form-antibiotique #libelle_nature').val(details[1]);    
         ...
         $(document).on('click', '#addA', function() {
            ...
            $('.modal-titleA').text('Ajouter un nouvel Echantillon');
  ...
  $('#footerA').on('click', '.ajout', function() {
    ...
    $(this).attr("disabled","disabled");
        $.ajax({
            type: 'post',
            dataType:'json',
            url: "{{route('dashboard_nature_echantillon')}}",
      ...
      .done(function(msg){
                        if(typeof msg['erreur'] !== 'undefined'){
                             $(":input:not('[name=_token],[type=checkbox],[type=radio]')").val('');
                              ...
                              var data = [
                               "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_nature + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_nature + "</a>"
                              ];
     ...
     $('#footerA').on('click', '.edit', function() {
      ...
      .done(function(msg){
                        ...
                        tr.find('td:eq(0)').html(  "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_nature + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_nature + "</a>");
