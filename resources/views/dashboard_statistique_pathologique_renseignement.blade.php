...
@section('d_content')
 ...
 <script type="text/javascript">
    ...
    valid = valid && checkEmpty($("#TR"));
     valid = valid && checkEmpty($("#age_min"));
     valid = valid && checkEmpty($("#age_max"));
    valid = valid && checkEmpty($("#RC"));
    return valid; 
  }
...
<div class="tab-content">
               <legend> <h4 class="text-left" style="color:rgb(51,122,183); font-weight: bold;">STATISTIQUE DES PATHOLOGIES PAR RENSEIGNEMENT CLINIQUE</h4></legend>

   <div class="row" style="margin-bottom:15px; padding-top:10px;margin-top:15px; height: 250px;border-radius:10px;background-color:rgb(238, 247, 240);">
            <div class="col-sm-5">
              <div class="row" style="">
                <div class="col-sm-6">
          ...
          <div class="row">
             <div class="form-group">
                   <label for="RC" class="control-label col-sm-8 " style="padding-left:45px">Renseignement clinique : <br /><span class="RC-validation validation-error"></span></label>
                  <div class="col-sm-11" style="padding-left:45px">
                      <select class="form-control" name="RC" id="RC" >
                       <option class="text-center" value="" selected="true">--- Choix du renseignement ---</option> 
                       @foreach($renseignement as $item)
...
function getExportFileName(){
 ...
 return 'STATISTIQUE PATHOLOGIQUE RENSEIGNEMENT CLINIQUE ' + $("#analyse option:selected").text();
}
var table =  $('#tabSP').DataTable({
    ...
    $(".dt-button buttons-excel").addClass("btn btn-success");
     ...
     $("#btnPrint").click(function(){
           ...
           window.open("{{ route('dashboard_print_stat_renseignement_clinique') }}?date_debut="+$("#du").val()+"&date_fin="+$("#au").val()+"&genre="+$("#genre").val()+"&examen="+$("#analyse").val()+"&rendu="+$("#element").val()+"&resultats[]="+ tab +"&renseignement="+ $("#RC").val() +"&age_min="+ $("#age_min").val()+"&age_max="+ $("#age_max").val());
        ...
        if(validate()){
         $('#tabSP').dataTable().fnClearTable();
            ...
            $("#TR option:selected").each(function () {
             ...
             $.ajax({
                            url: "{{route('dashboard_get_statistique_pathologie_renseignement_clinique')}}",
                            dataType: "json",
                            type: "get",
                             beforeSend: function(){
                                     $('.blockMe').block({ 
              ...
              .done(function(msg){
                  ...
                  var colonne3 = "";
                  if(2 < elmts.length){
                       colonne1 = elmts[2].valeur;
                  }        
                  if(3 < elmts.length){
                       colonne2 = elmts[3].valeur;
                  }
                  if(4 < elmts.length){
                       colonne3 = elmts[4].valeur;
                  }
                            var data = [
                                elmts[0].valeur,
                                 elmts[1].valeur,
