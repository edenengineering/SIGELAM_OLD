...
<script type="text/javascript">
 function validate() {
    ...
    var valid = true;
    valid = checkEmpty($("#libelle_profession"));
    return valid; 
  }
...
<div class="tab-content">
	<div class="row" style="margin-bottom:15px; margin-top:15px;">
          ...
          <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">LISTE DES PROFESSIONS</h4>
          </caption>  
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              <th class="text-center">Nom de la profession</th>
            </tr>
          </thead>
           @foreach($professions as $item)
              <tr id="item{{$item->id}}">
                ...
                <td class="text-left"><a class="edit-modalA" style="cursor:pointer"
                    data-info="{{$item->id}},{{$item->libelle_profession}}">
                    <span class="glyphicon glyphicon-edit"></span> {{$item->libelle_profession}}
                  </a>
                </td>
              </tr>
          @endforeach
				  ...
				  <div class="modal-dialog">
				    ...
				    <div class="modal-content">
				      <div class="modal-header" style="background-color:rgb(103,184,120); color:white;">
				        ...
				        <h2 id="titre" class="modal-titleA" >Ajouter une Profession</h2>
				      </div>
				      <div class="modal-body">
				        <form  id="form-antibiotique" class="form-horizontal" >
				        	<div class="row">
                    <div class="form-group ">
                      <label for="libelle_profession" class="control-label col-sm-5">Nom de la profession (*):&nbsp;<br /><span class="libelle_profession-validation validation-error"></span></label>
                      <div class="col-sm-6">
                        <input  class="form-control" name="libelle_profession" id="libelle_profession" type="text" />
                      </div>
                    </div>
                    
                  </div>
                  
                  <input type="hidden" id="id_profession" name="id_profession" value="">
				        </form>
				      </div>
   ...
   $('div.dataTables_filter input').focus();
     ...
     $('form input').on('keypress', function(e) {
        ...
        $(document).on('click', '.edit-modalA', function() {
            ...
            $('.modal-titleA').text("Modifier les informations de la profession");
            
            
            var details = $(this).data('info').split(',');
            $('#form-antibiotique #id_profession').val(details[0]);
    
            $('#form-antibiotique #libelle_profession').val(details[1]);    
         ...
         $(document).on('click', '#addA', function() {
            ...
            $('.modal-titleA').text('Ajouter une nouvelle Profession');
  ...
  $('#footerA').on('click', '.ajout', function() {
    ...
    $(this).attr("disabled","disabled");
        $.ajax({
            type: 'post',
            dataType:'json',
            url: "{{route('dashboard_profession')}}",
      ...
      .done(function(msg){
                        if(typeof msg['erreur'] !== 'undefined'){
                             $(":input:not('[name=_token],[type=checkbox],[type=radio]')").val('');
                              ...
                              var data = [
                               "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_profession + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_profession + "</a>"
                              ];
     ...
     $('#footerA').on('click', '.edit', function() {
      ...
      .done(function(msg){
                        ...
                        tr.find('td:eq(0)').html(  "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_profession + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_profession + "</a>");
