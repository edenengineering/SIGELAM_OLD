...
<div class="tab-content">
  ...
  <table id="tabCAA" style="width:100%; heigth:60%" class="table table-striped table-responsive blockMe">
           <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">CHIFFRE D'AFFAIRE ANNUEL</h4>
          </caption>   
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              ...
              <th class="text-center">Total CA</th>
            </tr>
          </thead>
...
$('#tabCAA').DataTable({
                ...
                buttons: [
                        {
                          ...
                          className: 'btnExcel',
                          title : "EVOLUTION CHIFFRE D'AFFAIRE ANNUEL"
                        }
    ...
    $(".dataTables_filter input").addClass("form-control");
      ...
      if(validate()){
               $.ajax({
                        url: "{{route('dashboard_evolution_ca_annuel_get')}}",
                        dataType: "json",
                        data: {
                    ...
                    .done(function(msg){
                              ...
                              $("#total").val(formatMoney(JSON.parse(msg['total'])));
                               for(i=0;i < tab.length;i++){
                                ...
                                var rowIndex = $('#tabCAA').dataTable().fnAddData(data);
                                var row = $('#tabCAA').dataTable().fnGetNodes(rowIndex);
                                $(row).attr( 'id','item' + i); 
                                var tr = $("#tabCAA tr#item"+i);
                               ...
                               if($("#tabCAA").dataTable().fnSettings().aoData.length===0) {
     ...
     $('#btnPrint').click(function(){
            ...
            window.open("{{ route('dashboard_evolution_ca_annuel_impression') }}?annee="+$("#annee").val());
    });
@endsection