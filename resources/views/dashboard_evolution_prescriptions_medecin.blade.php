...
<div class="tab-content">
  ...
  <table id="tabEPM" style="width:100%; heigth:60%" class="table table-striped table-responsive blockMe">
           <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">EVOLUTION PRESCRIPTIONS MEDECINS</h4>
          </caption>   
...
@section('another')
 var tableD = $('#tabEPM').DataTable({
               ...
               "serverSide": true,
               "ajax" : {
                  "url" : "{{ route('dashboard_get_evolution_prescriptions') }}",
                  "dataType" : "json",
                  "type" : "POST",
                   "data": function ( d ) {
                    return $.extend( {}, d, {
    ...
    $('.mdcA1').click(function(){
     ...
     $('#btnPrintA').click(function(){
          if( ($("#tabEPM").dataTable().fnSettings().aoData.length!==0) ){  
            window.open("{{ route('dashboard_evolution_prescriptions_medecin_impression') }}?annee_debut="+ $("#annee_debut").val() +"&annee_fin="+$("#annee_fin").val());
          }else{
              $("#AlerPre1").modal({
                        keyboard: false,
                        show : true,
