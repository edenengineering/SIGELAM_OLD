...
<script src="{{ URL::asset('js/jquery.min.js') }}" ></script>     
 ...
 function validate() {
    ...
    var valid = true;
    valid = checkEmpty($("#libelle_type_materiel"));
    return valid; 
  }
...
<div class="tab-content">
  <div class="row" style="margin-bottom:15px; margin-top:15px;">
          ...
          <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">LISTE DES TYPES MATERIELS</h4>
          </caption>  
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              <th class="text-center">Nom du Type Materiel</th>
            </tr>
          </thead>
           @foreach($type_materiels as $item)
              <tr id="item{{$item->id}}">
                ...
                <td class="text-left"><a class="edit-modalTC" style="cursor:pointer"
                    data-info="{{$item->id}},{{$item->libelle_type_materiel}}">
                    <span class="glyphicon glyphicon-edit"></span> {{$item->libelle_type_materiel}}
                  </a>
                </td>
              </tr>
          @endforeach
          ...
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              <div class="modal-header" style="background-color:rgb(103,184,120); color:white;">
                ...
                <h2 id="titre" class="modal-titleT" >Ajouter un Type Materiel</h2>
              </div>
              <div class="modal-body">
                <form  id="form-TC" class="form-horizontal" >
                  <div class="row">
                    <div class="form-group ">
                      <label for="libelle_type_materiel" class="control-label col-sm-3">Nom du Type materiel (*):<br /><span class="nom_type_materiel-validation validation-error"></span></label>
                      <div class="col-sm-7">
                        <input  class="form-control" name="nom_type_materiel" id="libelle_type_materiel" type="text" />   
                      </div>
                    
                    </div>
                  </div>
                  
                  <input type="hidden" id="id_type_materiel" name="id_type_materiel" value="">
                </form>
              </div>
    ...
    $(".dataTables_filter input").addClass("form-control");
     ...
     $('form input').on('keypress', function(e) {
        ...
        $(document).on('click', '.edit-modalTC', function() {
            ...
            $('.modal-titleT').text('Modifier les informations du Type Materiel');
            
            
            var details = $(this).data('info').split(',');
            $('#form-TC #id_type_materiel').val(details[0]);
    
            $('#form-TC #libelle_type_materiel').val(details[1]);    
            $("#add_TC").modal({
                        keyboard: false,
                        show : true,
                        backdrop: "static",
         ...
         $(document).on('click', '#addTC', function() {
            ...
            $('.modal-titleT').text('Ajouter un nouveau Type Matériel');
  ...
  $('#footerTC').on('click', '.ajout', function() {
   if(validate()){
        $.ajax({
            type: 'post',
            dataType:'json',
            url: "{{route('dashboard_type_materiel')}}",
      ...
      .done(function(msg){
                        if(typeof msg['erreur'] !== 'undefined'){
                             $(":input:not('[name=_token],[type=checkbox],[type=radio]')").val('');
                              ...
                              var data = [
                               "<a class=\'edit-modalTC\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_type_materiel + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_type_materiel + "</a>"
                              ];
     ...
     if(validate()){
      ...
      .done(function(msg){
                        ...
                        tr.find('td:eq(0)').html(  "<a class=\'edit-modalTC\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_type_materiel + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_type_materiel + "</a>");
