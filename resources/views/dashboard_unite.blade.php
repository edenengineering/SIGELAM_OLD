...
<script type="text/javascript">
 function validate() {
    ...
    var valid = true;
    valid = checkEmpty($("#libelle_unite"));
    return valid; 
  }
...
<div class="tab-content">
	<div class="row" style="margin-bottom:15px; margin-top:15px;">
          ...
          <caption id="cap">
              <h4 class="text-center" style="color:rgb(51,122,183); font-weight: bold;">LISTE DES UNITES DE SOINS</h4>
          </caption>  
          ...
          <thead style="background-color: LightSlateGray; color:white;  -webkit-box-shadow: 0px -4px 3px rgba(50, 50, 50, 0.75);-moz-box-shadow:    0px -4px 3px rgba(50, 50, 50, 0.75);box-shadow:0px -4px 3px rgba(50, 50, 50, 0.75);">
            <tr  style ="heigth:10px;">
              <th class="text-center">Unité de soins</th>
            </tr>
          </thead>
           @foreach($unites as $item)
              <tr id="item{{$item->id}}">
                ...
                <td class="text-left"><a class="edit-modalA" style="cursor:pointer"
                    data-info="{{$item->id}},{{$item->libelle_unite}}">
                    <span class="glyphicon glyphicon-edit"></span> {{$item->libelle_unite}}
                  </a>
                </td>
              </tr>
          @endforeach
      ...
      <div id="add_antibiotique" class="modal fade" role="dialog">
				  <div class="modal-dialog">
                    ...
                    <div class="form-group ">
                      <label for="libelle_antibiotique" class="control-label col-sm-5">Libelle de l'unité de soins (*):&nbsp;<br /><span class="libelle_unite-validation validation-error"></span></label>
                      <div class="col-sm-6">
                        <input  class="form-control" name="libelle_unite" id="libelle_unite" type="text" />
                      </div>
                    </div>
   ...
   $('div.dataTables_filter input').focus();
     ...
     $('form input').on('keypress', function(e) {
        ...
        $(document).on('click', '.edit-modalA', function() {
            ...
            $('.modal-titleA').text("Modifier les informations de l'unité de soins");
            
            
            var details = $(this).data('info').split(',');
            $('#form-antibiotique #id_unite').val(details[0]);
    
            $('#form-antibiotique #libelle_unite').val(details[1]);    
         ...
         $(document).on('click', '#addA', function() {
            ...
            $('.modal-titleA').text('Ajouter un nouvel unité de soins');
    ...
    $(this).attr("disabled","disabled");
      ...
      .done(function(msg){
                        if(typeof msg['erreur'] !== 'undefined'){
                             $(":input:not('[name=_token],[type=checkbox],[type=radio]')").val('');
                              ...
                              var data = [
                               "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_unite + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_unite + "</a>"
                              ];
     ...
     $('#footerA').on('click', '.edit', function() {
      ...
      .done(function(msg){
                        ...
                        tr.find('td:eq(0)').html(  "<a class=\'edit-modalA\' style=\'cursor:pointer\' data-info=\'" + nv.id + "," + nv.libelle_unite + "\'><span class=\'glyphicon glyphicon-edit\'></span> "+ nv.libelle_unite + "</a>");
